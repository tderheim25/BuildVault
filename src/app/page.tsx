            ...
            <div className="flex items-center gap-4">
              <div className="relative flex-1 max-w-md">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 rounded-xl bg-white/10 backdrop-blur-sm border border-white/20 text-white placeholder:text-white/70 focus:outline-none focus:ring-2 focus:ring-white/50"
                />
              </div>
              <Button variant="ghost" size="icon" className="text-white hover:bg-white/10 rounded-xl">
                <Bell className="h-5 w-5" />
              </Button>
              <div className="h-10 w-10 rounded-full bg-white/20 backdrop-blur-sm border border-white/30"></div>
            </div>
          </div>
        </div>
          ...
          <Card className="lg:col-span-2 dashboard-card">
            <CardHeader>
              <div className="flex justify-between items-center">
                <div>
                  <CardTitle className="text-xl font-bold text-gray-900 mb-1">Recent Projects</CardTitle>
                  <CardDescription className="text-gray-600">Latest construction sites</CardDescription>
                </div>
                {(profile.role === 'admin' || profile.role === 'manager') && (
                  <Link href="/projects/new">
                    <Button className="gradient-primary hover:opacity-90 text-white rounded-xl shadow-md hover:shadow-lg transition-all duration-200 dashboard-button">
                      New Project
                    </Button>
                  </Link>
                )}
            ...
            <CardContent>
              {sites && sites.length > 0 ? (
                ...
                <div className="text-center py-12">
                  ...
                  {(profile.role === 'admin' || profile.role === 'manager') && (
                    <Link href="/projects/new">
                      <Button className="gradient-primary hover:opacity-90 text-white rounded-xl shadow-md hover:shadow-lg transition-all duration-200 dashboard-button px-8 py-6 text-base">
